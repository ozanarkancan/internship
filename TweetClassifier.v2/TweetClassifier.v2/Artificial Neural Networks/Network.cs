...
namespace TweetClassifier.v2.Artificial_Neural_Networks
{
    class Network
    {
            ...
            while (true)
            {
                ...
                foreach (Pattern p in trainSet)
                {
                    ...
                    for (int i = 52; i < 56; i++)
                    {
                        ...
                        error = outNodes[i - 52] - neurons[i].value;
                        if (error != 0)
                            errorCount++;
        ...
        public void trainBackPropagation()
        {
            initilizeWeight();
            int count = 0;
            while (true)
            {
                ...
                foreach (Pattern p in trainSet)
                {
                    ...
                    for (int i = 66; i < 70; i++)
                    {
                        ...
                        foreach (Link link in neurons[i].inputLinks)
                        {
                            link.weight = link.weight + learningRate * link.neurons[0].value * (neurons[i].value * (1 - neurons[i].value) * (outNodes[i - 66] - neurons[i].value));
                        }
                    }
                    ...
                    for (int i = 52; i < 65; i++)
                    {
                        foreach (Link link in neurons[i].inputLinks)
                        {
                            ...
                            link.weight = link.weight + learningRate * link.neurons[0].value;
                    ...
                    error = 0;
                    for (int i = 66; i < 70; i++)
                        error += Math.Pow(outNodes[i - 66] - neurons[i].value, 2);
